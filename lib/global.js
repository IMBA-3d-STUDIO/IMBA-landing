...
/**
 * 用于由客户端注入 Clerk 用户（SSG 时不调用 useUser，避免构建报错）
 */
export const SetClerkUserContext = createContext(null)
...
export function GlobalContextProvider(props) {
  ...
  // 登录验证：仅通过 ClerkUserInjector（client-only）注入，SSG 时不调用 useUser
  const enableClerk = !!process.env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY
  const [clerkUserState, setClerkUserState] = useState(CLERK_FALLBACK)
  const { isLoaded, isSignedIn, user } = clerkUserState
  ...
  return (
    <SetClerkUserContext.Provider value={enableClerk ? setClerkUserState : null}>
