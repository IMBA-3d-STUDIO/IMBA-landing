...
import { getPageWithRetry } from './getPostBlocks'
...
export async function getConfigMapFromConfigPage(allPages) {
  ...
  let pageRecordMap = await getPageWithRetry(configPageId, 'config-table')
  ...
  for (const table of ['Config-Table', 'CONFIG-TABLE']) {
    if (content) break
    pageRecordMap = await getPageWithRetry(configPageId, table)
  ...
  // 找到PAGE中的 inline database（排除页面自身，避免误用 page 块）
  const configTableId = content?.find(contentId => {
    if (contentId === configPageId) return false
    const b = pageRecordMap.block[contentId]
    return (
  ...
  if (!databaseRecordMap?.value) {
    console.error(`[Notion配置] 配置表块无有效数据: ${configTableId}`)
    return null
  }
