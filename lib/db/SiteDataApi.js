...
async function convertNotionToSiteData(SITE_DATABASE_PAGE_ID, from, pageRecordMap) {
  ...
  // 站点配置优先读取配置表格，否则读取blog.config.js 文件；解析失败时进入维护模式
  let NOTION_CONFIG = await getConfigMapFromConfigPage(collectionData)
  const maintenanceMode = NOTION_CONFIG == null
